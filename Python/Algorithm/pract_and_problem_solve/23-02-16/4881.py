...
def soonyeol(i, N, list1):
    ...
    if i == N: # 끝까지 다 더했으면
        ...
        for j in range(i, N):
            ...
            sum1 += num_list[i][list1[i]] # 현재 합에 i번째 항 더하기

            # 하지만 현재 더하고 있는 합이 최소합보다 크면 현재의 순열 조합 만들기 stop
            if sum1 >= min_sum:
                sum1 -= num_list[i][list1[i]]  # 복귀
                list1[i], list1[j] = list1[j], list1[i]  # 복귀
                continue

            soonyeol(i + 1, N, list1)
